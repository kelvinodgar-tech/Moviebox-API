...
//
// v5 additions (server-side renew + lifetime stats):
//   1) SERVER-SIDE RENEW. When a paused download resumes after the signed
//      CDN link expired, the relay used to 302 the download manager to the
//      site's re-resolve endpoint, which 302'd back with a fresh link.
//      Mobile download managers DROP the Range header across those
//      cross-origin redirects, and a range-less fresh link answers with a
//      full-file 200 that appending managers splice onto the partial file:
//      the classic "resumed download jumps from 25MB to 40MB and is
//      corrupt". The relay now completes the whole renew chain ITSELF:
//      it fetches the site's re-resolve endpoint (auto=1) with
//      redirect:manual, resolves the fresh token out of the redirect's
//      Location, and re-requests the upstream with the ORIGINAL Range -
//      the browser receives ONE clean 206/200 answer and never sees a
//      redirect. One internal renew per request; a second stale bounce
//      surfaces as a friendly error page instead of another bounce.
//   2) LIFETIME STATS. Counters (served/bytes/go/renews) persist to
//      stats.json next to this file so transfer totals survive restarts;
//      the status route reports both since-boot and lifetime numbers.
//
// v6 additions (verified chunked serving - merged from the undeployed v5
// chunk engine after the CF-worker front was removed by owner directive):
//   Live diagnosis (2026-09-24, prior session): the CDN INTERMITTENTLY
//   answers an open-ended ranged request (bytes=X-) with perfect headers
//   (206, correct Content-Range and Content-Length) but a body whose bytes
//   are NOT the file's bytes at X. A manager that resumes appends those
//   wrong bytes at the pause offset: full file size, "complete", corrupt
//   content. Bounded ranges (bytes=X-Y) tested correct every time. So:
//     - the relay NEVER sends the CDN an open-ended range; every transfer
//       is served in bounded 16 MiB slices with exact header checks;
//     - each slice's head is cross-checked against an independent tiny
//       bounded fetch - only a POSITIVE mismatch (two reads disagreeing)
//       stops a slice; an unverifiable check (429/5xx on the verify fetch)
//       proceeds, never fails a healthy transfer;
//     - a slice that ends short is re-fetched for just the missing tail;
//       exhausted retries destroy the client socket so the download
//       manager restarts/resumes instead of finalizing a corrupt file;
...
const RENEW_TIMEOUT_MS = 25000; // site re-resolve (scrape) can take a while
// v6 verified-chunk engine:
const CHUNK_SIZE = 16 * 1024 * 1024; // upstream slices are fetched in 16 MiB bounded ranges
const VERIFY_BYTES = 64 * 1024; // head of each chunk is cross-checked against an independent fetch
...
const stats = { served: 0, bytes: 0, errors: 0, go: 0, shortfalls: 0, renews: 0, chunkRetries: 0, chunkErrors: 0, verifyMismatches: 0 };

/* ===== lifetime stats (persisted across restarts) ======================= */
const STATS_FILE = path.join(path.dirname(fileURLToPath(import.meta.url)), "stats.json");
let lifetime = { served: 0, bytes: 0, go: 0, renews: 0, shortfalls: 0, savedAt: 0 };
try {
  lifetime = { ...lifetime, ...JSON.parse(fs.readFileSync(STATS_FILE, "utf8")) };
} catch {}
let statsSnapshot = { served: 0, bytes: 0, go: 0, renews: 0, shortfalls: 0 };
function persistStats() {
  for (const k of Object.keys(statsSnapshot)) {
    lifetime[k] = (lifetime[k] || 0) + (stats[k] || 0) - statsSnapshot[k];
  }
  statsSnapshot = { served: stats.served, bytes: stats.bytes, go: stats.go, renews: stats.renews, shortfalls: stats.shortfalls };
  lifetime.savedAt = Date.now();
  try {
    fs.writeFileSync(STATS_FILE, JSON.stringify(lifetime));
  } catch {}
}
...
/** Server-side renew: fetch the calling site's re-resolve endpoint ourselves
 * (the token's `c` path with auto=1), extract the FRESH link out of its
 * redirect, and return it as a new target. The browser never sees a
 * redirect: mobile download managers drop the Range header across
 * cross-origin redirects, and a range-less fresh link answers with a
 * full-file 200 that appending managers splice onto the partial file (the
 * classic corrupted-resume). Returns a fresh target or null. */
async function renewTarget(renewPath, clientRange) {
  const base = String(HOME).replace(/\/+$/, "");
  const sep = renewPath.includes("?") ? "&" : "?";
  const renewUrl = base + renewPath + sep + "auto=1";
  try {
    ...
    if (r.status >= 300 && r.status < 400) {
      ...
      if (loc) {
        try {
          const locUrl = new URL(loc, renewUrl);
          // The fresh /dl coordinates, whatever public origin they carry:
          // resolve the token LOCALLY instead of hairpinning over the net.
          const fresh = resolveTarget(locUrl.searchParams);
...
/** Stale upstream: the CDN refusing the link (403/429 = referer rejected,
 * 404/410 = expired signature) or serving an HTML error page on a 200. */
function isStaleResponse(resp) {
  if ([403, 429, 404, 410].includes(resp.status)) return true;
  if (resp.status === 200) {
...
async function fetchUpstream(target, rangeHeader, playReferer, controller) {
  const headers = {
    ...
    Accept: "*/*",
    Referer: playReferer,
    // v6: callers always pass a BOUNDED range (open-ended ranges are the
    // corruption vector this engine exists to avoid).
 ...
 * Verdicts: "ok" (bytes agree), "mismatch" (positive evidence of wrong
 * bytes - retry the chunk), "skip" (could not verify: 429/5xx/network on
 * the VERIFY fetch itself - proceed with the chunk, never fail a healthy
 * transfer for lack of a second opinion). */
async function verifyHead(upstreamUrl, playReferer, controller, offset, peek) {
  ...
  try {
    ...
    if (v.status !== 206) {
      await drainBody(v);
      return "skip"; // range ignored / busy: no usable second opinion
    }
    const vcr = parseContentRange(v.headers.get("content-range"));
    if (!vcr || vcr.start !== offset) {
      await drainBody(v);
      return vcr ? "mismatch" : "skip"; // answering from the wrong offset is corruption
    }
    const vbody = await readAtMost(v.body, n);
    const overlap = Math.min(vbody.length, n);
    if (overlap === 0) return "skip";
    return vbody.subarray(0, overlap).equals(peek.subarray(0, overlap)) ? "ok" : "mismatch";
  } catch {
    return "skip";
  }
}
...
async function relay(req, res, query, forceDownload) {
  let target = resolveTarget(query);
  ...
  // hop 0: the link the caller arrived with; hop 1: the internally renewed
  // link (only when hop 0 came back stale and the token carries renew
  // coordinates). One renew maximum per request: a second stale bounce
  // surfaces as a friendly error page, never as another bounce. The whole
  // renew chain runs INSIDE the relay (v5): the browser's download manager
  // receives a single clean answer and never follows the cross-origin
  // redirects that made range-dropping managers append a full-file 200 onto
  // their partial file (the classic corrupted resume).
  for (let hop = 0; hop < 2; hop++) {
    ...
    // ---- v6 step 1: size probe (bytes=0-0; open-ended ranges NEVER used) --
    // A 1-byte bounded probe both validates the link before any byte is
    // served and learns the total size + metadata in one cheap request.
    let total = null;
    let upMeta = {};
    {
      const connectTimeout = setTimeout(abortUpstream, CONNECT_TIMEOUT_MS);
      let probe;
      try {
        probe = await fetchUpstream(upstreamUrl, "bytes=0-0", playReferer, controller);
      } catch (e) {
      ...
      clearTimeout(connectTimeout);
      if (isStaleResponse(probe)) {
        await drainBody(probe);
        stats.errors++;
        if (target.renew && !renewed) {
          renewed = true;
          stats.renews++;
          const fresh = await renewTarget(target.renew, clientRange);
          if (fresh) {
            target = fresh;
            continue; // hop 1: probe the fresh link
          }
        }
    ...
    // ---- v6 step 2: resolve the client's requested slice ------------------
    let start = 0;
    ...
    if (total === null) {
      // No total could be determined (CDN ignored the range probe): fall
      // back to the legacy single-stream pass-through. Happens only on CDNs
      // without range support, where byte offsets are meaningless anyway.
      return legacyStream(req, res, upstreamUrl, clientRange, playReferer, target,
        forceDownload, nonce, controller, abortUpstream, isHead);
    }
    if (start >= total) {
    ...
    // ---- v6 step 4: verified chunked transfer -------------------------------
    // Every byte served comes from a BOUNDED upstream range whose response
    // headers were checked (206 + Content-Range starting exactly where we
    // asked), whose head was cross-checked against an independent fetch, and
    // whose delivery is exact - short bodies trigger a tail refetch, and a
    // transfer that cannot be completed cleanly is DESTROYED so the download
    // manager retries instead of finalizing a corrupt file.
    let served = 0;
    const fail = () => {
      stats.errors++;
      try {
    ...
    for (let c0 = start; c0 <= end; ) {
      ...
      for (let attempt = 0; attempt <= CHUNK_RETRIES && !ok; attempt++) {
        if (res.destroyed || res.writableEnded) return;
        if (attempt > 0) stats.chunkRetries++;
        let up;
        try {
        ...
        if (isStaleResponse(up)) {
          // Mid-transfer staleness cannot renew (headers are already on the
          // wire): destroy so the download manager re-requests, and the NEXT
          // relay pass takes the server-side renew before any byte is served.
          await drainBody(up);
          stats.chunkErrors++;
          fail();
          return;
        ...
        // Peek the chunk head, then cross-check it with an independent tiny
        // bounded fetch (see verifyHead for the verdict semantics).
        ...
        const verdict = await verifyHead(upstreamUrl, playReferer, controller, c0, head);
        if (verdict === "mismatch") {
          stats.verifyMismatches++;
          endIter(it);
          continue; // corruption signature: fresh-connection retry
        }
        // verdict "ok" or "skip": proceed - only positive evidence of
        // corruption stops a chunk, never the lack of a second opinion.
        ...
        while (written < want) {
          if (res.destroyed || res.writableEnded) return;
          if (++tailRounds > CHUNK_RETRIES + 1) { fail(); return; } // no infinite tail loops
          ...
          const tverdict = await verifyHead(upstreamUrl, playReferer, controller, missingFrom, thead);
          if (tverdict === "mismatch") {
            stats.verifyMismatches++;
            endIter(tit);
            continue;
          }
    ...
    // Exact delivery: a clean end is only allowed when every promised byte
    // was verified and written (shortfall => destroy, never finalize short).
    if (served < promised) {
      stats.shortfalls++;
      fail();
      return;
  ...
  // Unreachable in practice: every loop path either serves or returns.
  return sendJson(req, res, 500, { error: "No upstream response" });
}

/** Legacy single-stream relay for range-less CDNs (no total size known):
 * the pre-v6 pass-through with exact-size headers and the shortfall guard. */
async function legacyStream(req, res, upstreamUrl, clientRange, playReferer, target,
  forceDownload, nonce, controller, abortUpstream, isHead) {
  const connectTimeout = setTimeout(abortUpstream, CONNECT_TIMEOUT_MS);
  let up;
  try {
  ...
  if (status === 206 && rangeInfo) {
    ...
    if (!clientRange && rangeInfo.start === 0 && rangeInfo.end === rangeInfo.total - 1) {
      // Client asked for no range but got a full-file 206: deliver it as a
      // clean 200 with the total size so progress shows "25 / 357 MB".
      status = 200;
  ...
  // Shortfall guard: clean-end short of the promise destroys the socket so
  // managers retry with a Range request instead of finalizing truncated.
  let written = 0;
  ...
  try {
    ...
    if (!ROOT || pathname === ROOT) {
      return sendJson(req, res, 200, {
        ...
        lifetime: {
          served: lifetime.served,
          bytes: lifetime.bytes,
          go: lifetime.go,
          renews: lifetime.renews,
          shortfalls: lifetime.shortfalls,
        },
      });
    }
