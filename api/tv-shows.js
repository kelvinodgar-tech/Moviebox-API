...
// Returns ALL TV shows (no filter). Paginated.
// Uses the backend /subject/filter with sort="ForYou" and filters to subjectType=2.
  ...
  try {
    ...
    while (results.length < limit && currentPage <= maxPages) {
      const r = await fetch(`${API}/wefeed-h5api-bff/subject/filter`, {
        ...
        body: JSON.stringify({ page: currentPage, perPage: 30, sort: "ForYou" }),
      ...
      for (const s of items) {
        if (s.subjectType !== 2) continue; // only TV shows
        const n = norm(s);
