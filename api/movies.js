...
// Returns ALL movies (no filter). Paginated.
// Uses the backend /subject/filter with classify="movie" and filters to subjectType=1.
  ...
  try {
    ...
    while (results.length < limit && currentPage <= maxPages) {
      const r = await fetch(`${API}/wefeed-h5api-bff/subject/filter`, {
        ...
        body: JSON.stringify({ page: currentPage, perPage: 30, sort: "Latest" }),
      ...
      for (const s of items) {
        if (s.subjectType !== 1) continue; // only movies
        const n = norm(s);
